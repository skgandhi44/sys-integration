<?php
    ...
    //connect to db
    $mysqli = new mysqli("127.0.0.1","","","");//Your DB info here (localhost, mysql root, mysql pass, database)
    ...
    //takes arguments from bash scripts
        ...
        if ($argv[1] == "package"){
            ...
            $statement = "INSERT INTO packages(path, status) VALUES ('$argv[2]', '$argv[3]');";
        ...
        elseif ($argv[1] == "extract" && $argv[4] == "latest"){
            ...
            $statement = "SELECT path FROM packages WHERE status='working' ORDER BY version DESC LIMIT 1;";
        ...
        elseif ($argv[1] == "extract" && $argv[4] == "QA"){
            ...
            $statement = "SELECT path FROM packages WHERE status='QA' ORDER BY version DESC LIMIT 1;";
        ...
        elseif ($argv[1] == "extract" && $argv[4] == "V"){
            ...
            $statement = "SELECT path FROM packages WHERE version='$argv[5]';";
        }
    ...
    elseif ($result->num_rows > 0) {
        ...
        while($row = mysqli_fetch_assoc($result)) {
            //echo "path: ".$row["path"]."\r\n";
            $pathdir = $row["path"];
            file_put_contents("/home/db-qa/bin/latest",$pathdir);
